...
export default function TeamFinderPage() {
	...
	const filteredUsers = users.filter((user) => {
		...
		const matchesHackathon =
			selectedHackathon === '' || selectedHackathon === 'all' || user.hackathons.includes(selectedHackathon)
				...
				<div className='max-w-4xl mx-auto'>
					...
					<Card className='border border-gray-200 rounded-md mb-8 bg-white'>
						<CardHeader className='border-b border-gray-200 p-4'>
							<CardTitle className='text-lg font-semibold text-black'>Search Participants</CardTitle>
						</CardHeader>
						<CardContent className='p-4'>
							<div className='space-y-4'>
								...
								<div className='flex items-center gap-2'>
									...
									/>
									<Label htmlFor='lookingForTeam' className='text-sm text-gray-800'>
					...
					<div className='space-y-6'>
						<div className='flex justify-between items-center'>
							<h2 className='text-xl font-semibold text-black'>Results ({filteredUsers.length})</h2>
							<Button
								variant='outline'
								size='sm'
						...
						{filteredUsers.length > 0 ? (
							<div className='grid grid-cols-1 gap-4'>
								{filteredUsers.map((user) => (
									<Card key={user.id} className='border border-gray-200 rounded-md bg-white'>
												...
												<div className='flex-1'>
													<div className='flex flex-col md:flex-row md:items-center md:justify-between gap-2 mb-2'>
														<div className='flex items-center gap-2'>
															...
															{user.lookingForTeam && (
																<Badge className='border border-gray-300 text-black px-2 py-1 text-xs rounded-md'>
																	Looking for team
																</Badge>
															)}
														</div>
							...
							<div className='text-center py-12 bg-gray-50 rounded-md'>
								<img src='/users.svg' className='h-12 w-12 text-gray-400 mx-auto mb-4' alt='No users' />
